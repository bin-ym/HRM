...
try {
    $stmt = $conn->prepare("SELECT username, email, last_login FROM users WHERE id = ? AND role = 'Applicant'");
    $stmt->execute([$applicant_id]);
    $applicant = $stmt->fetch(PDO::FETCH_ASSOC);

    if (!$applicant) {
        throw new Exception("Applicant data not found for ID: $applicant_id");
    }
